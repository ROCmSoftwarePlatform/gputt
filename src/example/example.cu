...
static void test()
{
  ...
  for (int i = 0; i < NumTransposeMethods; i++)
  {
    ...
    if (GPUTT_SUCCESS == gputtPlan(
      &plan, 4, dim, permutation, sizeof(idata[0]), 0, method))
      plans.push_back(plan);
  }
  
  // Option 2: Create plan on NULL stream and choose the method based on heuristics
  // GPUTT_ERR_CHECK(gputtPlan(&plan, 4, dim, permutation, sizeof(idata[0]), 0));
  // plans.push_back(plan);

  // Option 3: Create plan on NULL stream and choose the method based on performance measurements
